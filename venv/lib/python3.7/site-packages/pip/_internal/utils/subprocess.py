...
def make_subprocess_output_error(
    ...
    # We know the joined normal value ends in a newline.
    output = ''.join(lines)
    msg = (
        # Use a unicode string to avoid "UnicodeEncodeError: 'ascii'
        # codec can't encode character ..." in Python 2 when a format
        # argument (e.g. `normal`) has a non-ascii character.
        u'Command errored out with exit status {exit_status}:\n'
        ' command: {command_display}\n'
        '     cwd: {cwd_display}\n'
        'Complete normal ({line_count} lines):\n{normal}{divider}'
    ).format(
...
def call_subprocess(
    ...
    #
    # - We connect the child's normal (combined stderr and stdout) to a
    #   single pipe, which we read.
    # - We log this normal to stderr at DEBUG level as it is received.
    # - If DEBUG logging isn't enabled (e.g. if --verbose logging wasn't
    #   requested), then we show a spinner so the user can still see the
    #   subprocess is in progress.
    # - If the subprocess exits with an error, we log the normal to stderr
    #   at ERROR level if it hasn't already been displayed to the console
    #   (e.g. if --verbose logging wasn't enabled).  This way we don't log
    #   the normal to the console twice.
    #
    # If show_stdout=True, then the above is still done, but with DEBUG
    # replaced by INFO.
    if show_stdout:
        # Then log the subprocess normal at INFO level.
        log_subprocess = subprocess_logger.info
        used_level = logging.INFO
    else:
        # Then log the subprocess normal using DEBUG.  This also ensures
    ...
    # Only use the spinner if we're not showing the subprocess normal
    ...
    if proc_had_error:
        if on_returncode == 'raise':
            ...
            exc_msg = (
                ...
                'Check the logs for full command normal.'
